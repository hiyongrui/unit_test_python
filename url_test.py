...
class UrlTest(unittest.TestCase):
    ...
    #self, url, scheme, path, domain_name, query, fragment, port, user, password
    """
    ...
    def test_to_string(self):
        ...
        url = Url(address)
        # self.assertEqualUrl(url, "http", "/x/y/z", "example.com", "a=b#s", "s", 123, "user", "password") #error fix 1, query not suppose to have #
        self.assertEqualUrl(url, "http", "/x/y/z", "example.com", "a=b", "s", 123, "user", "password")
        self.assertEqual(
            str(url), address
        )
